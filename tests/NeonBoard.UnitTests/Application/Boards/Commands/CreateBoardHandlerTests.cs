...
public class CreateBoardHandlerTests
{
    ...
    [Fact]
    public async Task Handle_WithExplicitPrefix_ShouldReturnBoardDtoWithPrefix()
    {
        var command = new CreateBoardCommand(Guid.NewGuid(), "Sprint Board", "SPR");

        var result = await _handler.Handle(command, CancellationToken.None);

        result.Prefix.Should().Be("SPR");
    }

    [Fact]
    public async Task Handle_WithoutPrefix_ShouldReturnBoardDtoWithAutoGeneratedPrefix()
    {
        var command = new CreateBoardCommand(Guid.NewGuid(), "Sprint Board");

        var result = await _handler.Handle(command, CancellationToken.None);

        result.Prefix.Should().Be("SB");
    }
}
