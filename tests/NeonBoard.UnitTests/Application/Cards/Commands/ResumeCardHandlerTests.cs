...
public class ResumeCardHandlerTests
{
    private readonly IBoardRepository _boardRepository = Substitute.For<IBoardRepository>();
    private readonly ResumeCardHandler _handler;

    public ResumeCardHandlerTests()
    {
        _handler = new ResumeCardHandler(_boardRepository);
    }

    [Fact]
    public async Task Handle_WithValidCommand_ShouldReturnResumedCardDto()
    {
        ...
        var command = new ResumeCardCommand(projectId, board.Id, cardId);
    ...
    public async Task Handle_WhenBoardNotFound_ShouldThrowNotFoundException()
    {
        ...
        var command = new ResumeCardCommand(Guid.NewGuid(), boardId, Guid.NewGuid());
    ...
    public async Task Handle_WhenProjectIdMismatch_ShouldThrowNotFoundException()
    {
        ...
        var command = new ResumeCardCommand(differentProjectId, board.Id, board.Cards[0].Id);
