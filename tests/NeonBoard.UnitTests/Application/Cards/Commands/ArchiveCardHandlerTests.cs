...
public class ArchiveCardHandlerTests
{
    private readonly IBoardRepository _boardRepository = Substitute.For<IBoardRepository>();
    private readonly ArchiveCardHandler _handler;

    public ArchiveCardHandlerTests()
    {
        _handler = new ArchiveCardHandler(_boardRepository);
    }

    [Fact]
    public async Task Handle_WithValidCommand_ShouldReturnArchivedCardDto()
    {
        ...
        var command = new ArchiveCardCommand(projectId, board.Id, cardId);
    ...
    public async Task Handle_WhenBoardNotFound_ShouldThrowNotFoundException()
    {
        ...
        var command = new ArchiveCardCommand(Guid.NewGuid(), boardId, Guid.NewGuid());
    ...
    public async Task Handle_WhenProjectIdMismatch_ShouldThrowNotFoundException()
    {
        ...
        var command = new ArchiveCardCommand(differentProjectId, board.Id, board.Cards[0].Id);
